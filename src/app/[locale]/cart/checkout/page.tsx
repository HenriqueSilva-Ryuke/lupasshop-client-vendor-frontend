  ...
  const handleFinishOrder = async () => {
    ...
    try {
      for (const [storeId, storeItems] of itemsByStore) {
        ...
        if (!storeId || storeId === 'unknown' || storeId.length < 32) {
          console.error('Invalid storeId, skipping order creation for items:', storeItems);
          continue; // Skip invalid legacy cart items to prevent GraphQL crash
        }
