  ...
  ({ variant = 'default', size = 'md', className = '', asChild = false, children, ...rest }, ref) => {
    ...
    const renderContent = () => {
      if (variant === 'inset') {
        return (
          <>
            <span
              aria-hidden
              className="absolute inset-0 z-0 pointer-events-none transition-[clip-path] duration-500 ease-out bg-zinc-900 dark:bg-white"
      ...
      if (variant === 'slide') {
        return (
          <>
            <span className="absolute inset-0 z-0 bg-zinc-900 dark:bg-white translate-y-[101%] group-hover:translate-y-0 transition-transform duration-300 ease-out" />
            <span className="relative z-10 flex items-center gap-2 group-hover:text-white dark:group-hover:text-black transition-colors duration-300">
              {children}
            </span>
          </>
    ...
    const variantClasses = {
      ...
      outline: 'border-zinc-900 dark:border-white text-zinc-900 dark:text-white bg-transparent',
      inset: 'relative overflow-hidden border-zinc-900 dark:border-white text-zinc-900 dark:text-white bg-transparent',
      slide: 'relative overflow-hidden border-zinc-900 dark:border-white text-zinc-900 dark:text-white bg-transparent',
      icon: 'p-2 border-2 border-zinc-200 dark:border-zinc-800 rounded-lg hover:border-primary transition-colors text-zinc-900 dark:text-white',
    };
