...
export function AddButtonMobile({ onClick, icon = 'add' }: Omit<AddButtonProps, 'label'>) {
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.9 }}
      onClick={onClick}
      className="md:hidden fixed bottom-28 right-4 w-12 h-12 bg-blue-600 text-white rounded-full flex items-center justify-center hover:bg-blue-700 transition-colors shadow-lg z-40"
      title="Add"
    >
